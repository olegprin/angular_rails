...
module ApplicationHelper
  ...
  def dont_show_head 
   params[:controller]=="answerfrommoderators" || params[:controller]=="devise/sessions" || params[:controller]=="admin/admins" || params[:controller]=="admin/configurables"
  end
...
def resource_class
  ...
  def info_who_created(resourse)
    @info_for_user=Info.find_by(user_id: resourse.user_id)
    
  end
  

  def user_ret_email(resourse)
    @email=User.find(resourse.user_id).email.downcase
    return @email
  end
